...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await nylas.threads.list({
      identifier: grantId,
      queryParams: queryParams as Parameters<typeof nylas.threads.list>[0]["queryParams"],
    })

    const firstPage = await response
    const items = firstPage.data || []
    const threads: EmailThread[] = items.map((t) =>
      formatThread(t as Parameters<typeof formatThread>[0])
    )

    return NextResponse.json(threads)
  } catch (err) {
    console.error("Nylas threads error:", err)
    return NextResponse.json(
